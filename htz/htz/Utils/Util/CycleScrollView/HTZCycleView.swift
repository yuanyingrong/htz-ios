...
public enum HTZPageControlStyle {
    case center                         // 中间
...
// MARK: - ICycleView常量
private struct HTZCycleViewConst {
    static let cellIdentifier = "HTZCycleViewCell"        // Cell标识
    static let pageControlHeight: CGFloat = 19          // 指示器高度
}

// MARK: - ICycleViewDelegate 代理方法
@objc public protocol HTZCycleViewDelegate: NSObjectProtocol {
    
    // 图片点击
    @objc optional func htzCycleView(cycleView: HTZCycleView, didSelectItemAt index: Int)
    
    // 图片自动滚动
    @objc optional func htzCycleView(cycleView: HTZCycleView, autoScrollingItemAt index: Int)
    
    // 自定义Cell
    @objc optional func htzCycleView(cycleView: HTZCycleView, collectionView: UICollectionView, cellForItemAt indexPath: IndexPath, picture: String) -> UICollectionViewCell
}


public class HTZCycleView: UIView {
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        collectionView.register(HTZCycleViewCell.self, forCellWithReuseIdentifier: HTZCycleViewConst.cellIdentifier)
        return collectionView
    }()
    
    // 图片滚动指示器
    private lazy var pageControl: UIPageControl = {
        let pageControl = UIPageControl(frame: CGRect(x: 0, y: bounds.height-HTZCycleViewConst.pageControlHeight, width: bounds.width, height: HTZCycleViewConst.pageControlHeight))
    ...
    // TODO: 3.外部访问属性
    // 代理
    public weak var delegate: HTZCycleViewDelegate?
    ...
    public override func layoutSubviews() {
        ...
        pageControl.frame = CGRect(x: 0, y: bounds.height-HTZCycleViewConst.pageControlHeight, width: bounds.width, height: HTZCycleViewConst.pageControlHeight)
    }
     ...
     */
    open var pageControlStyle: HTZPageControlStyle = .center {
        didSet {
            ...
            case .bottom(let bottom):
                pageControl.center.y = bounds.height - HTZCycleViewConst.pageControlHeight/2 - bottom
            }
        }
    }
    ...
    deinit {
        print("HTZCycleView deinit success")
    }
    
}

// MARK: - UICollectionViewDataSource, UICollectionViewDelegate
extension HTZCycleView: UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if isCustomCell {
            // 自定义Cell
            return delegate?.htzCycleView?(cycleView: self, collectionView: collectionView, cellForItemAt: IndexPath(item: indexPath.item % pictures.count, section: 0), picture: pictures[indexPath.item % pictures.count]) ?? UICollectionViewCell()
        } else {
            // 默认Cell
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: HTZCycleViewConst.cellIdentifier, for: indexPath) as! HTZCycleViewCell
    ...
    public func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        delegate?.htzCycleView?(cycleView: self, didSelectItemAt: indexPath.item % pictures.count)
    }
...
extension HTZCycleView {
    ...
    @objc private func updateCollectionViewAutoScrolling() {
        if let indexPath = collectionView.indexPathsForVisibleItems.last {
            ...
            if indexPath.item + 1 < pictures.count * 2 {
                ...
                print("HTZCycleView 定时器方法，更新Cell位置 \(nextPath)")
            }
        }
    }
    ...
    public func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        if pageControl.currentPage != page {
            ...
            delegate?.htzCycleView?(cycleView: self, autoScrollingItemAt: page)
        }
    }
    
}

class HTZCycleViewCell: UICollectionViewCell {
